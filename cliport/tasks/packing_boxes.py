...
class StackingBoxes(Task):
    ...
    def __init__(self):
        super().__init__()
        self.max_steps = 12
        self.lang_template = "stack all the boxes onto the wooden pallet"
        self.task_completed_desc = "done stacking boxes."

        self.stack_level = 2  # 1 or 2 or 3
        self.n_boxes = 8      # maximum number of boxes
    ...
    def reset(self, env):
        ...
        zone_size = (0.14, 0.14, min_object_dim * self.stack_level)
        ...
        def KDTree(node, SKIP_THRESHOLD=0.0):
            size = node.bbox[3:] - node.bbox[:3]
        ...
        for bbox in bboxes:
            ...
            if len(colors) == 0:
                colors = [utils.COLORS[c] for c in utils.COLORS if c != 'brown']
                print("=== Color out of used, reuse one ===")
        ...
        if len(true_ids) > self.n_boxes:
            print("=== Too many boxes, ignore the excess ===")
            true_ids = true_ids[:self.n_boxes]
            true_poses = true_poses[:self.n_boxes]
